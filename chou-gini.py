...
def comp_gQt(setIdx1,setIdx2,node,l,ct,lOrder,n,f21,f22):
	gini1 = 0
	gini2 = 0
...
def comp_gIdx(node):
	...
	count = count_data(l4)
	gIdx = 0
	for l,ct in l4.items():
		gIdx += (ct/float(count))**2
	gIdx = 1-gIdx
	return gIdx
...
def prtitn_node(node,n,opt,prt1=[],prt2=[]):
	...
	if opt=='dev':
		...
		while A!=A_0:
			...
			for l,ct in ln.items():  ### for each beta...
				...
				[qt1,qt2] = comp_gQt(A,A_bar,node,l,ct,lOrder,n,f21,f22)
	...
	if opt=='tes':
		...
		gIdx1 = comp_gIdx(subset1)
		gIdx2 = comp_gIdx(subset2)
		gIdx = (ct1*gIdx1+ct2*gIdx2)/float(count)
		return subset1,subset2,part1,part2,gIdx1,gIdx2,gIdx,ct1,ct2
...
def chou_split(node):
	...
	for i in range(3):
		# print '========================='
		# print '$ l'+str(i+1)
		[ss1,ss2,pt1,pt2,gIdx1[i],gIdx2[i],gIdx[i],ct1[i],ct2[i]] = prtitn_node(node,i+1,'dev')
...
gini_hld_0 = comp_gIdx(hld_4gm)
# print 'Held-out Gini:',gini_hld_0
ndGidx = [gini_hld_0]
...
while 1:
	...
	ndHld = hld_node[nd]
	ndTes = tes_node[nd]
	gini_hld_0 = ndGidx[nd]
	count = ct_nd[nd]
	ndPrd_trn = prd_trn[nd]  ### with smoothing ###

	# print count
	# print 'Hld Gini before partition:',gini_hld_0

	[sbst1_dev,sbst2_dev,ques,part1,part2,ct1_dev,ct2_dev] = chou_split(ndDev)
	[sbst1_hld,sbst2_hld,pt1_hld,pt2_hld,gini_hld1,gini_hld2,gini_hld,ct1,ct2] = prtitn_node(ndHld,ques+1,'hld',part1,part2)
	[sbst1_tes,sbst2_tes,ct1_tes,ct2_tes] = prtitn_node(ndTes,ques+1,'tes',pt1_hld,pt2_hld)
	
	red_hld = (gini_hld_0-gini_hld)*count/float(hld_ct)
	print 'Hld Gini reduction:',red_hld
	...
	if red_hld>thrsh:
		...
		ndGidx.append(gini_hld1)
		ndGidx.append(gini_hld2)
		ndGidx.pop(nd)
...
# np.savetxt('perplexity-chou-gini.csv',prplx)
